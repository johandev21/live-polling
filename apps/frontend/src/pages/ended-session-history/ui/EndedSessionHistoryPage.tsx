  ...
  return (
    <ul className="flex flex-col gap-2">
      {poll.choiceResults.map((result) => (
        <li key={result.id}>
          <Card className="p-4"><div aria-label={`${result.label}: ${result.percentage}%`} className="flex flex-col gap-2" role="group"><div className="flex items-baseline justify-between gap-4 text-sm"><span className="font-semibold">{result.label}</span><span className="font-mono text-xs font-semibold text-primary">{result.percentage}%</span></div><Progress className="h-2" value={result.percentage} /><span className="text-xs text-muted-foreground">{result.count} responses</span></div></Card>
        </li>
      ))}
    </ul>
  );
        ...
        <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_24rem]">
          ...
          <aside className="flex flex-col gap-4 xl:sticky xl:top-6">
            <SessionTotals history={history} />
             <Card aria-labelledby="host-results-heading" className="flex items-start gap-3 p-4">
              <LockKeyhole
                aria-hidden="true"
                className="mt-0.5 shrink-0 text-primary"
                size={17}
